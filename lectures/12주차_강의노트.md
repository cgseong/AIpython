...
```python
"""
문제: 여러 학생의 성적 정보를 처리하는 함수를 작성하세요.
- 함수명: process_student_data
- 매개변수: **students (학생명=점수 형태의 키워드 인수들)
- 반환값: 딕셔너리 (전체 통계 + 각 학생별 학점 정보)
"""

# 해결 방안
def process_student_data(**students):
    """
    여러 학생의 성적을 종합 처리하는 함수
    
    Parameters:
    - **students: 학생명=점수 형태의 키워드 인수들
    
    Returns:
    - dict: 종합 통계 및 개별 학생 정보
    """
    if not students:
        return {"error": "학생 데이터가 없습니다"}
    
    # 이전에 정의한 get_grade 함수 재사용
    def get_grade(score):
        if not (0 <= score <= 100):
            return "잘못된 점수"
    ...
    for name, score in students.items():
        if isinstance(score, (int, float)) and 0 <= score <= 100:
            student_details[name] = {
                ...
                "status": "통과" if score >= 70 else "재수강 필요"
            }
            valid_scores.append(score)
        else:
            student_details[name] = {"error": "유효하지 않은 점수"}
    ...
    if valid_scores:
        statistics = {
            ...
            "pass_count": sum(1 for score in valid_scores if score >= 70),
            "fail_count": sum(1 for score in valid_scores if score < 70)
        }
    else:
        statistics = {"error": "유효한 점수 데이터가 없습니다"}
...
result = process_student_data(
    김철수=95, 이영희=87, 박민수=76, 최지은=92, 홍길동=68, 송미래=98
)

print("=== 성적 처리 결과 ===")
print(f"전체 통계: {result['statistics']}")
print("\n개별 학생 정보:")
for name, info in result['student_details'].items():
    print(f"{name}: {info}")
```
...
```python
"""
문제: 계산기 팩토리 함수를 작성하세요.
- 함수명: create_calculator
- 매개변수: initial_value (초기값)
- 반환값: 계산 기능들을 포함한 딕셔너리
- 요구사항: 상태를 유지하며 연산 이력을 기록하는 계산기
"""

# 해결 방안
def create_calculator(initial_value=0):
    """
    상태를 유지하는 계산기를 생성하는 팩토리 함수
    
    Parameters:
    - initial_value (float): 계산기의 초기값
    
    Returns:
    - dict: 계산기 기능들을 포함한 딕셔너리
    """
    # 클로저 변수들 (Enclosing Scope)
    ...
    def add(value):
        ...
        current_value += value
        operation_history.append(f"{old_value} + {value} = {current_value}")
    ...
    def divide(value):
        nonlocal current_value
        if value == 0:
            operation_history.append(f"오류: 0으로 나누기 시도")
            return None
    ...
    def get_history():
        return operation_history.copy()  # 원본 보호를 위한 복사본 반환
    
    def reset(new_value=0):
        nonlocal current_value
        old_value = current_value
        current_value = new_value
        operation_history.append(f"리셋: {old_value} → {new_value}")
        return current_value
    
    def clear_history():
        nonlocal operation_history
        operation_history = []
        return "기록이 초기화되었습니다"
...
# 계산기 인스턴스 생성
calc1 = create_calculator(10)
calc2 = create_calculator(0)

# calc1 연산 테스트
print(f"calc1 초기값: {calc1['value']()}")
calc1['add'](5)
calc1['multiply'](3)
calc1['subtract'](10)

print(f"calc1 최종값: {calc1['value']()}")
print("calc1 연산 기록:")
for record in calc1['history']():
    print(f"  {record}")

# calc2 연산 테스트
print(f"\ncalc2 초기값: {calc2['value']()}")
calc2['add'](100)
calc2['divide'](4)
calc2['multiply'](2)

print(f"calc2 최종값: {calc2['value']()}")
print("calc2 연산 기록:")
for record in calc2['history']():
    print(f"  {record}")

# 에러 처리 테스트
print(f"\n0으로 나누기 테스트: {calc1['divide'](0)}")
```
...
#### 해결책
```python
def safe_append_function(item, target_list=None):
...
#### 문제 상황
```python
def calculation_without_return(x, y):
...
#### 해결책
```python
def calculation_with_proper_return(x, y):
...
**다음 학습 목표**: 객체지향 프로그래밍(OOP)과 클래스(Class) 개념으로의 확장
